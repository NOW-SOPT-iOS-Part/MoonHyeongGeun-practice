...
final class WelcomeViewController: UIViewController {
    ...
    private let goMainButton: UIButton = {
        ...
        button.makeRoundBorder(cornerRadius: 6)
        return button
    }()
    
    private lazy var backLoginButton: UIButton = {
        let button = UIButton(frame: CGRect(x: 20, y: 498, width: 335, height: 58))
        button.backgroundColor = UIColor(resource: .grey)
        button.setTitle("다시 로그인", for: .normal)
        button.setTitleColor(.white, for: .normal)
        button.titleLabel?.font = UIFont.pretendardFont(weight: 700, size: 18)
        button.makeRoundBorder(cornerRadius: 6)
        button.addTarget(self, action: #selector(backLoginButtonDidTap), for: .touchUpInside)
        return button
    }()
    ...
    override func viewDidLoad() {
        ...
        self.view.addSubviews(logoImageView, welcomeLabel, goMainButton, backLoginButton)
